...
MIN_SEARCH_TOKEN = 3  # shorter words match everything and mean nothing
...
def search_products(
    ...
    with db.connection() as conn:
        ...
        matches = [
            ...
            if _matches_all_tokens(product, tokens)
...
def _matches_all_tokens(product: db.Product, tokens: list[str]) -> bool:
    """Every query token appears in the title or description, case-insensitively."""
...
def find_order(ctx: AuthContext, query: str) -> dict[str, Any]:
    ...
    if not query_tokens:
        return {
            "ok": False,
            "error": "invalid_argument",
            "reason": "query must name a product",
        }
    # The scope comes from the authenticated context, never from the query.
    if ctx.role == "shopper":
        ...
        if ctx.store_id is None:
            return {
                "ok": False,
                "error": "invalid_argument",
                "reason": "merchant context has no store id",
            }
    ...
    with db.connection() as conn:
        ...
        # Match across the whole authorized scope, then take the first five:
        # truncating first would hide older matches behind recent orders.
        matches = [
            order
            for order in db.list_order_search_candidates(conn, **scope)
            if _product_matches(titles.get(order.product_id, ""), query_tokens)
        ]
        return {"ok": True, "orders": [order.to_public_dict() for order in matches[:5]]}


def _search_tokens(text: str) -> list[str]:
    """Lowercase word tokens worth matching on, singulars normalized.

    Short filler ('a', 'my') carries no product meaning and would match
    everything, so tokens under three characters are dropped.
    """
    words = re.findall(r"[a-z0-9]+", text.lower())
    return [_singular(word) for word in words if len(word) >= MIN_SEARCH_TOKEN]


def _singular(word: str) -> str:
    """Crude plural fold so "earmuffs" finds "Earmuff"."""
    return word[:-1] if len(word) > 3 and word.endswith("s") else word


def _product_matches(title: str, query_tokens: list[str]) -> bool:
    """True when the product title shares a word with the query.

    Case-insensitive token matching, the simple option the handout allows.
    Swap in rapidfuzz.fuzz.partial_ratio here for typo tolerance; the rest
    of the tool does not change.
    """
    return bool(set(_search_tokens(title)) & set(query_tokens))
